...
export type ICommandeState = IPaginationBaseState;

export class Commande extends React.Component<ICommandeProps, ICommandeState> {
  state: ICommandeState = {
  ...
  render() {
    const { commandeList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="commande-heading">
          Commandes
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp; Create new Commande
          </Link>
        </h2>
            ...
            <tbody>
              {commandeList.map((commande, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={commande.date} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{commande.client ? <Link to={`client/${commande.client.id}`}>{commande.client.nom}</Link> : ''}</td>
